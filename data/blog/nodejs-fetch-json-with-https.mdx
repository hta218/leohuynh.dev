...
Tuy nhiên nếu **server** của bạn đơn thuần chỉ làm `API` cho client App và bây giờ bạn nhận 1 task nhỏ cần phải `request` đến 1 file `json` từ server khác, hoặc đơn giản là fetch 1 external `API` thì bạn sẽ chọn xài thư viện nào? <Twemoji emoji="thinking-face" />
...
```js {1,3,5,7,8,10,14} showLineNumbers
...
- Line `1`: **https** là built-in module của Node.js nên chúng ta có thể `require` luôn mà không cần install
- Line `3`: Tạo **request** với `https.get(url[, options][, callback])`
- Trong **callback** chúng ta sẽ listen các event của response với `res.on()`
- Line `5`: Mỗi lần `res.on("data")` trigger, cộng thêm `data` vào string `body`
- Line `8`: Kết thúc với `res.on("end")`, lúc này chỉ cần `JSON.parse(body)` là sẽ có JSON data mong muốn
- Line `7` + `10`: Lưu ý để parse trong `try {} catch() {}` để bắt lỗi nếu không **parse** được **json**!
- Line `14`: Nếu không tạo được **request** thì event `error` sẽ được trigger
