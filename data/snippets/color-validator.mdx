...
Strings like `unset`, `initial`, `inherit`, `transparent` are also valid values, so if you want to exclude these strings, just change the function a bit:
...
const isValidColor = color => {
  ...
  return !!otpNode.style.color && !/(unset|initial|inherit|transparent)/.test(color)
}
