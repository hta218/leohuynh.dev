...
Replace the code in line 9 with the `code` value that you saved in the previous step.

Replace the `base64 encoded client_id:client_secret` in line 4 with the base64 encoded value of your Spotify app `Client ID` and `Client secret`.
