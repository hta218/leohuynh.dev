// Pure formatters + brand-icon mapping for LLM/token-burn data. No DOM or env
// access, so it's safe to import from both the client rail renderer and the
// server-rendered `/llms` page.
...
export const INLINE_MODEL_ICONS: Record<string, string> = {
  openai:
...
export function modelBrandIcon(
  model: string,
): { type: 'cdn'; slug: string } | { type: 'inline'; key: string } {
...
// The brand icon already conveys the vendor — drop the "claude-" prefix so
// "claude-opus-4-8" reads as "opus-4-8".
